...
public final class SimpleTerrainShader {
    ...
    /*
     * Object visibility is decided with the same integer camera-space projection
     * used by Mesh.render(), rather than inferring RS winding from mirrored GL NDC.
     * Faces touching the RS 50-unit near plane are left to the GPU clipper; the
     * ordinary all-front path now uses RS's exact fixed-point yaw/pitch transforms,
     * 512/z screen projection, integer truncation, and signed-area test.
     */
    private static final String GEOMETRY =
            ...
            "uniform int uObjectPass; uniform ivec3 uRsCamera; uniform ivec2 uRsYawSinCos; uniform ivec2 uRsPitchSinCos;\n" +
            "bool rsProject(vec3 world,out ivec2 screen){\n" +
            " ivec3 w=ivec3(round(world));int x=w.x-uRsCamera.x;int y=(-w.y)-uRsCamera.y;int z=w.z-uRsCamera.z;\n" +
            " int k=(z*uRsYawSinCos.x+x*uRsYawSinCos.y)>>16;z=(z*uRsYawSinCos.y-x*uRsYawSinCos.x)>>16;x=k;\n" +
            " k=(y*uRsPitchSinCos.y-z*uRsPitchSinCos.x)>>16;z=(y*uRsPitchSinCos.x+z*uRsPitchSinCos.y)>>16;y=k;\n" +
            " if(z<50)return false;screen=ivec2((x*512)/z,(y*512)/z);return true;}\n" +
            "bool rsRejectFace(){if(uObjectPass==0)return false;ivec2 a;ivec2 b;ivec2 c;bool af=rsProject(vsWorldPos[0],a);bool bf=rsProject(vsWorldPos[1],b);bool cf=rsProject(vsWorldPos[2],c);if(!(af&&bf&&cf))return false;int area=(a.x-b.x)*(c.y-b.y)-(a.y-b.y)*(c.x-b.x);return area<=0;}\n" +
    ...
    private int rsCameraLocation=-1,rsYawSinCosLocation=-1,rsPitchSinCosLocation=-1;
    ...
    public void init(GL3 gl){
        ...
        rsCameraLocation=gl.glGetUniformLocation(program,"uRsCamera");rsYawSinCosLocation=gl.glGetUniformLocation(program,"uRsYawSinCos");rsPitchSinCosLocation=gl.glGetUniformLocation(program,"uRsPitchSinCos");
    ...
    public void setRsCamera(GL3 gl,int x,int z,int height,int yawSin,int yawCos,int pitchSin,int pitchCos){
        if(rsCameraLocation>=0)gl.glUniform3i(rsCameraLocation,x,height,z);
        if(rsYawSinCosLocation>=0)gl.glUniform2i(rsYawSinCosLocation,yawSin,yawCos);
        if(rsPitchSinCosLocation>=0)gl.glUniform2i(rsPitchSinCosLocation,pitchSin,pitchCos);
    }
    ...
    public void dispose(GL3 gl){textures.dispose(gl);if(program!=0){gl.glDeleteProgram(program);program=0;viewProjectionLocation=-1;textureCountLocation=-1;objectPassLocation=-1;rsCameraLocation=-1;rsYawSinCosLocation=-1;rsPitchSinCosLocation=-1;}}
}
